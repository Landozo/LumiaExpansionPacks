# Lando's Lucid Loom Stuff 

> [!WARNING]
> This page is currently out of date, I need to prep it for being all about Prolix's new Lumiverse client, which hosts new versions of his lucid loom preset in it instead. More information about it can be found at https://lumiverse.chat

Everything below is Archived/Out of Date, I need to update it for the new Lumiverse client!

---

THE LOOM GUIDE (For Sillytavern only/Archived/Outdated):
[The Loom Guide](https://github.com/Landozo/LumiaExpansionPacks/blob/main/Guides/LoomGuide.md)

LANDO'S Sillytavern M.U.G. (Mostly Useful Guide):
[LANDO SMUG](https://github.com/Landozo/LumiaExpansionPacks/blob/main/Guides/LandoSMUG.md)

From now on, my Lumia personality, author, and utility packs will all be downloaded using Prolix's new [Lumiverse Helper](https://github.com/prolix-oc/SillyTavern-LumiverseHelper) extension from the included browse packs button in said extension! Do not download them from this github!

My Author Packs, Utility Packs, and Lumia packs can all be downloaded from there!

## How to install my final releases of Lumia Packs!
1. Get the Lumiverse Helper Extension from the link above
2. Click "browse lucid packs" in extension settings
3. Hit download on my packs!

## WIP Planning:
- Planning (Defs started, not yet full pack)
- Testing (Raws available here in folders, testing personalities, format cleanup)
- Released (Released on Prolix's Lumiverse Helper extension!)

### [Personality Packs](PersonalityPacks)

- Pack 1 Released
- Pack 2 Released
- Pack 3 Released
- Pack 4 Released
- NSFW Pack 1 Testing
- Pack 5 Planning (Redundant Personalities)
- At least 5 other packs on the way

### [Author Packs](AuthorPacks)
- Author Pack 1 Released (It Starts)
- Author Pack 2 Released (Western Classics)
- Author Pack 3 Released (Non-Western Ancient classics)
- Author Pack 4 Released (Contemporary Authors)
- Author Pack 5 Testing (Scifi Pack)
- Author Pack 6 Planning (Shitpost Pack)
- Author Pack 7 Planning (Fantasy Pack)
- More on the way!

### [Utility Packs](UtilityPacks)
- Loom Utility Pack 1 Released
- Loom NSFW Utility Pack 1 Testing

### [Cards](Cards)
- Lumia Half OOC Visit Released

### Lorebooks
- Gnosis System Testing
- User Backstory Madlibs Planning
- User and Char Powers Testing
- World modifier Lorebooks Testing